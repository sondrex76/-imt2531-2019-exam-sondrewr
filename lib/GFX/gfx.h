...
namespace Gfx
{
    namespace Detail
    {
        ...
        RenderUniformVariable(GLuint program, GLenum type, const char* name, GLint location)
        {
            ...
            {
                case GL_FLOAT:
                    GFX_INTROSPECTION_GENERATE_VARIABLE_RENDER(float, 1, GL_FLOAT, glGetUniformfv, glProgramUniform1fv, ImGui::DragFloat);
                    break;

                case GL_FLOAT_VEC2:
                    GFX_INTROSPECTION_GENERATE_VARIABLE_RENDER(float, 2, GL_FLOAT_VEC2, glGetUniformfv, glProgramUniform2fv, ImGui::DragFloat2);
                    break;

                case GL_FLOAT_VEC3:
                    GFX_INTROSPECTION_GENERATE_VARIABLE_RENDER(float, 3, GL_FLOAT_VEC3, glGetUniformfv, glProgramUniform3fv, ImGui::DragFloat3);
                    break;

                case GL_FLOAT_VEC4:
                    GFX_INTROSPECTION_GENERATE_VARIABLE_RENDER(float, 4, GL_FLOAT_VEC4, glGetUniformfv, glProgramUniform4fv, ImGui::DragFloat4);
                    break;

                case GL_SAMPLER_2D:
                    GFX_INTROSPECTION_GENERATE_VARIABLE_RENDER(int, 1, GL_SAMPLER_2D, glGetUniformiv, glProgramUniform1iv, ImGui::InputInt);
                    break;
...
namespace Gfx
{
    const char* GLEnumToString(GLenum e)
    {
        ...
        switch (e)
        {
            ...
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_COMPLETE);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_UNDEFINED);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_UNSUPPORTED);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE);
            GFX_TO_STRING_GENERATOR(GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS);
        }
